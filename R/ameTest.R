...
#' for this model.
#' 
#' @usage ameTest(Y, Xdyad=NULL, Xrow=NULL, Xcol=NULL, rvar = !(model=="rrl") ,
...
#' 
#' data(YX_frn) 
#' fit<-ameTest(YX_frn$Y,YX_frn$X,burn=5,nscan=5,odens=1,model="frn")
#' # you should run the Markov chain much longer than this
#' 
#'  
#' @export ameTest
ameTest<-function (Y,Xdyad=NULL, Xrow=NULL, Xcol=NULL, 
...
{ 
  ...
  # 
  if(nscan %% odens!=0){
    stop('nscan must be multiple of odens')
  }
  ...
  { 
    ...
    {  
      ...
      if(gof){ Ys[is.na(Y)]<-NA ; GOF[((s-burn)/odens)+1,]<-gofstats(Ys) }
  ...
  if(!symmetric) 
  {
    ...
    fit <- list(BETA=BETA,VC=VC,APM=APM,BPM=BPM,U=U,V=V,UVPM=UVPM,EZ=EZ,
                YPM=YPM,GOF=GOF, 
                zList=zList, ysList=ysList, uList=uList, vList=vList)
  }
